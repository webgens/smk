...
<%
'*****************************************************************************************'
'JoinChgMemCertification.asp - 회원 가입(정회원 전환) - 본인인증
'Date		: 2018.12.19
'Update		:
'*****************************************************************************************'
...
JoinType		 = sqlFilter(Request("JoinType"))
IF JoinType = "" THEN
	...
	<script type="text/javascript">
		function after_JoinAuth(rCode, rMessage) {
			if (rCode == "OK") {
				...
				openAlertLayer("confirm", rMessage, "closePop('confirmPop', '')", "closePop('confirmPop', '');SDupInfoChk();");
			}
			else {
		...
		function move_Join() {
			//openPop("loading");
			location.href = "/ASP/Member/JoinChgMem.asp";
		}
	</script>

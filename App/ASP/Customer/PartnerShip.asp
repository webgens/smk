...
<!-- #include virtual="/INC/Header.asp" -->
	...
	<script type="text/javascript">
		function partnerShipWrite(){
			var wrtName = alltrim($("input[name='WrtName']", "form[name='PartnerShipForm']").val());
			if (wrtName.length == 0) {
			...
			var hp1 = $("select[name='HP1']", "form[name='PartnerShipForm']").val();
			if (hp1.length == 0) {
				openAlertLayer("alert", "휴대폰번호를 선택하여 주십시오.", "closePop('alertPop', 'HP1');", "");
				return;
			}

			var hp2 = $("input[name='HP2']", "form[name='PartnerShipForm']").val();
			if (hp2.length == 0) {
			...
			var hp3 = $("input[name='HP3']", "form[name='PartnerShipForm']").val();
			if (hp3.length == 0) {
			...
			var email = alltrim($("input[name='Email']", "form[name='PartnerShipForm']").val());
			if (email.length == 0) {
			...
			var company = alltrim($("input[name='Company']", "form[name='PartnerShipForm']").val());
			if (company.length == 0) {
				openAlertLayer("alert", "업체명을 입력하여 주십시오.", "closePop('alertPop', 'Company');", "");
				return;
			}

			var brandName = alltrim($("input[name='BrandName']", "form[name='PartnerShipForm']").val());
			if (brandName.length == 0) {
				openAlertLayer("alert", "브랜드명을 입력하여 주십시오.", "closePop('alertPop', 'BrandName');", "");
				return;
			}

			var contents = alltrim($("#Contents", "form[name='PartnerShipForm']").val());
			if (contents.length == 0) {
				openAlertLayer("alert", "문의 내용을 입력하여 주십시오.", "closePop('alertPop', 'Contents');", "");
				return;
			}

			var filename = alltrim($("input[name='FileName']", "form[name='PartnerShipForm']").val());
			...
			var clauseAgree = $("input[name='clauseAgree']", "form[name='PartnerShipForm']").prop("checked");
			if (!clauseAgree) {
			...
			//PartnerShipForm.submit();
			var form = $("#PartnerShipForm")[0];
			...
			$.ajax({
				...
				error		 : function (data) {
								closePop('loading');
								openAlertLayer("alert", "입점/제휴문의 입력 처리중 오류가 발생하였습니다.", "closePop('alertPop', '');", "");
				}
			});
            ...
            <div class="slider-for">
                <section>
                    <div class="customer-form">
                        <div class="h-line">
                            <h2 class="h-level4">입점/제휴문의</h2>
                            <p>최신 트렌드의 대한민국 대표 슈즈멀티샵 개척자인 슈마커와 여러분의 성공 비지니스를 함께 하세요.</p>
                        </div>

						<form name="PartnerShipForm" id="PartnerShipForm" method="post" enctype="multipart/form-data" action="Ajax/PartnerShipAddOk.asp">
                        <div class="form-field">
                            <fieldset>
                                <legend>필수입력 정보</legend>
                                <div class="fieldset">
                                    <label class="fieldset-label">문의유형</label>
                                    <div class="fieldset-row">
                                        <span class="select is-expand">
                                            <select id="Category" name="Category">
                                                <option value="상품 입점">상품 입점</option>
                                                <option value="마케팅 제휴">마케팅 제휴</option>
                                                <option value="기타">기타</option>
                                            </select>
                                            <span class="value">상품 입점</span>
                                        </span>
                                    </div>
                                </div>
                                <div class="fieldset">
                                ...
                                <div class="fieldset">
                                    ...
                                    <div class="fieldset-row">
                                        <span class="input is-expand">
                                            <input type="text" name="Email" id="Email" maxlength="50" title="이메일 입력" placeholder="이메일 입력">
                                        </span>
                                    </div>
                                </div>
                                <div class="fieldset">
                                ...
                                <div class="fieldset">
                                    <label class="fieldset-label">브랜드명</label>
                                    <div class="fieldset-row">
                                        <span class="input is-expand">
                                            <input type="text" name="BrandName" id="BrandName" maxlength="20" title="브랜드명 입력" placeholder="브랜드명 입력">
                                        </span>
                                    </div>
                                </div>
                            </fieldset>
                            <fieldset>
                                <legend>업체 상세정보</legend>
                                <div class="fieldset">
                                    <label class="fieldset-label">홈페이지</label>
                                    <div class="fieldset-row">
                                        <span class="input is-expand">
                                            <input type="text" name="Homepage" id="Homepage" maxlength="30" title="홈페이지 URL 입력" placeholder="홈페이지 URL 입력">
                                        </span>
                                    </div>
                                </div>
                                <div class="fieldset">
                                    ...
                                    <div class="fieldset-row">
                                        <span class="input is-expand">
                                            <input type="text" name="CompanyCate" id="CompanyCate" maxlength="25" title="카테고리 입력" placeholder="신발, 의류, 악세사리 등">
                                        </span>
                                    </div>
                                </div>
                                <div class="fieldset">
                                    <label class="fieldset-label">유통채널</label>
                                    <div class="fieldset-row">
                                        <span class="input is-expand">
                                            <input type="text" name="Distribution" id="Distribution" maxlength="25" title="유통채널 입력" placeholder="입점중인 유통채널 정보">
                                        </span>
                                    </div>
                                </div>
                                <div class="fieldset mb-15">
                                    ...
                                    <div class="fieldset-row">
                                        <span class="input is-expand textarea">
                                            <textarea name="Contents" id="Contents" title="문의 내용 입력" placeholder="입점/제휴를 원하시는 브랜드에 대한 간략한 설명과 제안 내용을 남겨주세요."></textarea>
                                        </span>
                                        <div class="upload">
...
%>
                            ...
                            <div class="buttongroup is-space">
                                <button type="button" onclick="partnerShipWrite();" class="button-ty2 button is-expand ty-red">등록하기</button>

...
If IsTopN = "Y" Then
	StoreProcName = "USP_Mobile_EShop_Street306_Product_Select_HistoryBack"
Else
	StoreProcName = "USP_Mobile_EShop_Street306_Product_Select"
End If
...
%>
								...
								<div class="area-pagination">
									<% If Cint(Page) > 1 Then %>
										<a href="javascript:getPrevPage('<%=PCode%>');" class="btn-prev1"><span class="hidden">이전 상품목록</span></a>
									<% Else %>
										<a nohref class="btn-prev"><span class="hidden">이전 상품목록</span></a>
									<% End If %>
									<% If Cint(Page) < Cint(PageCnt) Then %>
										<a href="javascript:getNextPage('<%=PCode%>');" class="btn-next"><span class="hidden">다음 상품목록</span></a>
									<% Else %>
										<a nohref class="btn-next1"><span class="hidden">다음 상품목록</span></a>
									<% End If %>
								</div>

...
WITH HTTP_Object
		...
		.Open				 "POST",			 "https://nid.naver.com/oauth2.0/token", False
		.SetRequestHeader	 "Content-Type",	 "application/x-www-form-urlencoded; charset=UTF-8"
		.Send				 "grant_type=authorization_code&client_id=" & NAVER_LOGIN_CLIENTID & "&client_secret=" & NAVER_LOGIN_CLIENTSECRET & "&code=" & Request("code")
		.WaitForResponse
...
IF AccessToken <> "" THEN
		...
		WITH HTTP_Object
				'API 통신 Timeout 을 30초로 지정
				.SetTimeouts 30000, 30000, 30000, 30000
				.Open				 "GET",						 "https://openapi.naver.com/v1/nid/me", False
		...
		IF ResponseText <> "" THEN
				...
				WITH ReadData
						...
						Email			 = .data("response").item("email")
						KName			 = .data("response").item("name")
						Gender			 = .data("response").item("gender")
						BirthDay		 = .data("response").item("birthday")
						Age				 = .data("response").item("age")
						NickName		 = .data("response").item("nickname")
						ProfileImage	 = .data("response").item("profile_image")

						If Not IsNull(KName) And KName <> "" Then
							KName			 = unicodeToKor(KName)
						End If
				END WITH
				SET ReadData = Nothing
		END IF
		...
		WITH oCmd
				...
				.Parameters.Append .CreateParameter("@SNSKind",	 adChar,	 adParamInput,  1, "N")
		...
		ELSE
				...
				ELSE
						...
						Response.Cookies("SNS_Kind")		= Encrypt("N")
...
%>
		<script type="text/javascript">
			var val = "<%=U_NUM%>///<%=ID%>///<%=Email%>///<%=Kname%>///N";
			APP_HistoryBack_SNS_Login(val);
		</script>
<%
ELSE
		Response.Redirect "/ASP/Error/ErrorPopupNone.asp?Title=SHOEMARKER&Msg=네이버 로그인 오류 입니다.<br />다시 시도해 주십시오.&Script=APP_PopupHistoryBack();"
		Response.End
END IF
%>